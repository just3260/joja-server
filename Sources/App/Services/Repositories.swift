...
extension Application {
    struct Repositories {
        struct Provider {
            static var database: Self {
                .init {
                    ...
                    $0.repositories.use { DatabaseStorageRepository(database: $0.db) }
                }
            }
        ...
        final class Storage {
            ...
            var makeTagRepository: ((Application) -> TagRepository)?
            var makeStorageRepository: ((Application) -> StorageRepository)?
            init() { }
        }
